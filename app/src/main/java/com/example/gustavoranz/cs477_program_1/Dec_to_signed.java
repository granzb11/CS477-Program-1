...
public class Dec_to_signed extends AppCompatActivity {

    Spinner middle_bits, right_bits;
    TextView question_TextView, results;
    EditText unsigned_answer;
    String num_bits, hex_bit1 ="0", hex_bit2="0", negative_decimal;
    int num_of_clicks, quiz_num, negative_decimalValue;
    boolean signed = false, too_small = false, too_big = false, hex_answer = false, quiz_num_too_small = false,
            quiz_num_too_big = false, quiz_num_hex = false;
    RadioGroup rg;
    Button check_answers_button, calculator_button;
    GridLayout myLayout;
    String binary_bit1, binary_bit2;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_dec_to_signed);
        ...
        myLayout = (GridLayout) findViewById(R.id.grid_layout_signed);
        ...
        else if(quiz_num > 127)
            quiz_num_too_big = true;
        else
            quiz_num_hex = true;

        question_TextView.setText("In signed, what is the 8-bit hex value for " + quiz_num + "?");
        ...
        if(!quiz_num_too_small & !quiz_num_too_big) {
            ...
            if(quiz_num < 0) {
                binary_bit1 = binary.substring((binary.length() - 8), (binary.length() - 4));
                binary_bit2 = binary.substring(binary.length() - 4, binary.length());
            }else{
                //getting the bits from the string
                binary_bit1 = binary.substring(0, (binary.length() - 4));
                binary_bit2 = binary.substring(binary.length() - 4, binary.length());
            }
            //getting hex representation
        ...
        check_answers_button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                ...
                if(num_of_clicks == 2){
                    ...
                    extras.putString("signed_answer", String.valueOf(signed));//attaching the boolean value of unsigned
                    back_to_main.putExtras(extras);//attaching the bundle to the intent
                    setResult(Activity.RESULT_OK,back_to_main);
                    finish();
                }else {
                    if (signed) {
                        Toast.makeText(Dec_to_signed.this, "Good job, you answered correctly!", Toast.LENGTH_SHORT).show();
